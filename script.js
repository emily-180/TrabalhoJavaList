 ...
 //--------------------------------------------------------------------------------------------
  ...
  function adicionarOrdenado() {
    ...
    } else if (novaPrioridade >= minhaLista.last().data.prioridade) {
      retorno = minhaLista.addLast(novaTarefa);
    } else if (novaPrioridade < minhaLista.first().data.prioridade) {
      retorno = minhaLista.addFirst(novaTarefa);
    } else {
      let noAtual = minhaLista.head.next;
      let indice = 1; // Começando do índice 1, pois já estamos no segundo nó
      while (noAtual !== null) {
        if (novaPrioridade < noAtual.data.prioridade) {
...
//--------------------------------------------------------------------------------------------
function mostrarInicioLista() {
  if (minhaLista.isEmpty()) {
    alert("Lista vazia!");
  } else {
    const tarefaInicio = minhaLista.first().data;
    alert(`Tarefa do início da lista: ${tarefaInicio.descricao}`);
  }
}
//----------------------------------------------------------------------------------------------
...
function removerElemento() {
  if(minhaLista.isEmpty()) 
    ...
    const tarefaRemovida = minhaLista.deleteFirst();
    let horaAgora=obterHoraAtual();
    let dataAgora=obterDataAtual();
    mostrarMensagemRemocao(tarefaRemovida, horaAgora, dataAgora);
    mostrarLista();
  }
}
//--------------------------------------------------------------------------------------------
function mostrarMensagemRemocao(tarefaRealizada, horaAgora, dataAgora) {
  const mensagem = document.getElementById("mensagem-remocao");
  let hora = calcularDiferencaHoras(tarefaRealizada.hora, horaAgora);
  let dia = calcularDiferencaDias(tarefaRealizada.data, dataAgora);
  mensagem.innerHTML ="Tarefa "+ tarefaRealizada.descricao + ", realizada em: "+dia + " dias e " +hora ; // apresentar a mensagem de remoção com dias e horas
...
function maisAntigo() {
  if (minhaLista.isEmpty()) {
    alert("Nenhuma tarefa é antiga, a lista está vazia.");
    return;
  }
  let tarefaMaisAntiga = minhaLista.head.data;
  let noAtual = minhaLista.head.next;
  while (noAtual !== null) {
    const tarefaAtual = noAtual.data;
    tarefaMaisAntiga = comparaTarefasDataHora(tarefaMaisAntiga, tarefaAtual);
    noAtual = noAtual.next;
  }
  alert(tarefaMaisAntiga);
}
//--------------------------------------------------------------------------------------------
 // funcao data
